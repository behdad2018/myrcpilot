...
int feedback_march(void)
{
	...
	***************************************************************************/
	...
	//B: trjectory defining
	// planned points based on the current location from mocab
	//double x[4] = {0.0, 1.0, 1.0, 0.0};
	//double y[4] = {0.0, 0.0, 1.0, 1.0};

	//double x[4] = {xbeeMsg.x, xbeeMsg.x+0.5, xbeeMsg.x+0.5, xbeeMsg.x};
	//double y[4] = {xbeeMsg.y, xbeeMsg.y, xbeeMsg.y+0.5, xbeeMsg.y+0.5};
	...
	//double xx= loc.xTraj[counter];
	//printf("%s%+5.2f", loc.xTraj[counter]);
				double pos_er_x;
				double pos_er_y;

				int lim=1000;
	// adding wait to each corner of squar, it stays for "lim" number of points
				if (counter==2){
					counter2=counter2+1;

					if (counter2<0.8*lim){
						setpoint.Z = -(1.0*counter2)/(lim*1.0) * 3.0 * 0.65;
					}
				...
				else if(counter==floor(1.0*nmax/4.0)){
					counter2=counter2+1;
					if (counter2<lim){
						counter = counter -1;
					}
				...
				else if(counter==floor(2.0*nmax/4.0)){
					counter2=counter2+1;
					if (counter2<lim){
						counter = counter -1;
					}
				...
				else if (counter==floor(3.0*nmax/4.0)){
					counter2=counter2+1;
					if (counter2<lim){
						counter = counter -1;
					}
					...
					counter2=counter2+1;	
					setpoint.Z = -(0.8*(lim-counter2))/(lim*1.0) * 3.0  * 0.65;
					if (counter2<lim){
						counter = counter -1;
					}
					else
...
// end of modifying the counter

				state_estimate.xp=loc.xTraj[counter];
				state_estimate.yp=loc.yTraj[counter];
// error definition
				pos_er_x = state_estimate.xp - xbeeMsg.x ;
				pos_er_y = state_estimate.yp - xbeeMsg.y ;

				////////////// PID for horizontal positon control

				// defining the acceptable band width
		...
		//	setpoint.Z = state_estimate.alt_bmp // set altitude setpoint to current altitude
					setpoint.Z = state_estimate.lidar;
